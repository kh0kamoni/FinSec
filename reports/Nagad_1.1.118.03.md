...
**Audit Subject**: `Nagad` (`com.konasl.nagad`)
...
| :--- | :--- |
| **Application Package** | `com.konasl.nagad` |
| **Version** | `1.1.118.03` (Code: `1218`) |
| **Binary Size** | 55.34 MB |
| **Binary SHA-256** | `849f074b8af0a887a3d5b3cbfe87fc745d26690d3b74f508b57c22cfad5ba3cf` |
| **Cybersecurity Maturity Score** | **46.0 / 100.0** |
...
- **Regulatory Observation**: Violation of Clause 4.1.5.19.c: Found 10 insecure ciphers (e.g. DES, RC4, ECB).
- **Evidence Findings**:
  * Insecure cipher algorithm detected: Insecure Cipher: DES
    ```
    Insecure Cipher: DES
    ```
  * Insecure cipher algorithm detected: Insecure Cipher: DES/ECB/noPadding
    ```
    Insecure Cipher: DES/ECB/noPadding
    ```
  * Insecure cipher algorithm detected: Insecure Cipher: DESede
    ```
    Insecure Cipher: DESede
    ```
  * Insecure cipher algorithm detected: Insecure Cipher: DESede/CBC/noPadding
    ```
    Insecure Cipher: DESede/CBC/noPadding
    ```
  * Insecure cipher algorithm detected: Insecure Cipher: AES/ECB/PKCS5Padding
    ```
    Insecure Cipher: AES/ECB/PKCS5Padding
    ```
  * Insecure cipher algorithm detected: Insecure Cipher: DESede/CBC/PKCS5Padding
    ```
    Insecure Cipher: DESede/CBC/PKCS5Padding
    ```
  * Insecure cipher algorithm detected: Insecure Cipher: RC2
    ```
    Insecure Cipher: RC2
    ```
  ...
  * Insecure cipher algorithm detected: Insecure Cipher: DES/ECB/NoPadding
    ...
    Insecure Cipher: DES/ECB/NoPadding
    ```
  * Insecure cipher algorithm detected: Insecure Cipher: DES/CBC/noPadding
    ```
    Insecure Cipher: DES/CBC/noPadding
    ```
...
- **Regulatory Observation**: Advisory under Clause 4.1.2.5: 4 functional permissions requested (READ_EXTERNAL_STORAGE, ACCESS_FINE_LOCATION, READ_CONTACTS, WRITE_EXTERNAL_STORAGE).
- **Evidence Findings**:
  * Functionally justified permission requiring operational governance: android.permission.READ_EXTERNAL_STORAGE
  * Functionally justified permission requiring operational governance: android.permission.ACCESS_FINE_LOCATION
...
- **Regulatory Observation**: Identified 8 unprotected exported application components.
- **Evidence Findings**:
  * Exported component without permission barrier: activity:com.konasl.dfs.ui.deeplink.IntentForwardingActivity
  ...
  * Exported component without permission barrier: activity:io.card.payment.CardIOActivity
  * Exported component without permission barrier: activity:io.card.payment.DataEntryActivity
...
- **Evidence Findings**:
  * Suspicious high-entropy string constant (6.21 bits/char): ABC...)_+
    ```
    ABC...)_+
    ```
  * Suspicious high-entropy string constant (6.00 bits/char): ABC...9-_
    ...
    ABC...9-_
    ```
  * Suspicious high-entropy string constant (4.53 bits/char): acc...$cp
    ```
    acc...$cp
    ```
